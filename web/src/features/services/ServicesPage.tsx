  ...
  return (
    <div className="mx-auto max-w-[1100px] px-6 py-6">
      <header className="mb-5 flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-xl font-semibold tracking-tight text-[var(--color-ink)]">Services</h1>
          <p className="text-sm text-[var(--color-ink-muted)]">
            {data ? `${data.length} connectors` : 'Connected services'}
          </p>
        </div>
        <div className="flex items-center gap-2">
          ...
          {canMutate && (
            <Button variant="primary" size="md" onClick={() => navigate('/services/new')}>
              <PlusIcon size={15} /> Add connector
            </Button>
          )}
        </div>
      </header>
      ...
      <Panel>
        {isLoading ? (
          ...
          <ErrorState description="Couldn't load services." onRetry={() => refetch()} />
        ) : rows.length === 0 ? (
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-[var(--color-line-soft)] text-left text-2xs uppercase tracking-wider text-[var(--color-ink-faint)]">
                ...
                <th className="hidden px-4 py-2.5 font-semibold md:table-cell">Endpoint</th>
                <th className="px-4 py-2.5 font-semibold">Status</th>
                <th className="px-4 py-2.5 text-right font-semibold">Last sync</th>
