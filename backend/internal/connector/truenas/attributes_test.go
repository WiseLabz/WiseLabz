...
func TestAttributeCatalogCoversEmittedKeys(t *testing.T) {
	fixtures := []struct {
		...
		{buildPools, poolsJSON},
		{buildDatasets, datasetsJSON},
		{buildDisks, disksJSON},
		{buildSMBShares, smbSharesJSON},
		{buildNFSShares, nfsSharesJSON},
		{buildServices, servicesJSON},
	...
	for _, f := range fixtures {
		_, entities, _ := f.build([]byte(f.raw))
