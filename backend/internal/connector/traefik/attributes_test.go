...
func TestAttributeCatalogCoversEmittedKeys(t *testing.T) {
	_, routers, _ := buildRouterTable([]byte(routersJSON))
	_, services := buildServiceTable([]byte(servicesJSON))
	_, middlewares := buildMiddlewareTable([]byte(middlewaresJSON))
	_, entryPoints := buildEntryPointTable([]byte(entryPointsJSON))

	emitted := map[string]map[string]string{}
	for _, group := range [][]connector.SnapshotEntity{routers, services, middlewares, entryPoints} {
		for _, e := range group {
